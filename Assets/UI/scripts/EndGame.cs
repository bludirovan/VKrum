...
public class EndGame : MonoBehaviour
{
    ...
    public LevelTimer levelTimer; // Ссылка на компонент LevelTimer
    ...
    private float remainingTime; // Сохранённое оставшееся время
    ...
    {
        if (other.CompareTag("Player") && !levelCompeted)
        {
            levelCompeted = true;
            CompleteLevel();
        }
    ...
    private void CompleteLevel()
    {
        ...
        remainingTime = levelTimer.StopTimerAndGetRemainingTime();

        winScreen.SetActive(true);
        winScreenAnimator.gameObject.SetActive(true);
        winScreenAnimator.SetTrigger(winScreenTrigger);

        // Используем remainingTime для расчёта оценки
        int nowGrade = CalcGrade(remainingTime);
        if (nowGrade > grade) grade = nowGrade;

        // Отображаем оставшееся время и пороговые времена
        DisplayTimes(remainingTime);
        ...
        if (grade != 0)
        {
            ...
            PlayerPrefs.SetFloat($"Level_{level.LevelName}_RemainingTime", remainingTime);
            PlayerPrefs.Save();
        }
    ...
    {
        // Отображаем оставшееся время в UI общего таймера
        if (gameTimeTexts != null)
            gameTimeTexts.text = FormatTime(playerTime);

        // Пороговые времена под звездами (конвертируем пороги в оставшееся время)
        float[] starThresholds = new float[]
        {
            level.timeGrade3,
            level.timeGrade2,
            level.timeGrade1
        ...
        for (int i = 0; i < timeTexts.Length && i < starThresholds.Length; i++)
        {
            if (timeTexts[i] == null) continue;
            timeTexts[i].text = FormatTime(starThresholds[i]);
        }
    }
